...
module Regular.Predicates.Applies.Soundness.Fixpoint (μσ : Sum) where
  ...
  AppAlμ-sound x ⟨ y ⟩ (peel d i r) hip
    with Zipper-match d x | inspect (Zipper-match d) x
  ...| nothing     | _ = Maybe-⊥-elim hip
  ...| just ⟨ x' ⟩ | [ X ] 
    with ⟪ r ⟫S x' | inspect ⟪ r ⟫S x'
  ...| mk | [ RX ]
    with trans (cong (λ P → (Zipper-inj i <$> (Maybe-map ⟨_⟩ P))) (sym RX)) hip
  ...| hip' with mk
  ...| nothing = Maybe-⊥-elim hip'
  ...| just k  
    rewrite just-inj (sym hip')
    with Zipper-match-inj-inv d x ⟨ x' ⟩ X
  ...| z' , prf rewrite prf = AppPeel z' d i r (AppS-sound x' k r RX) 
